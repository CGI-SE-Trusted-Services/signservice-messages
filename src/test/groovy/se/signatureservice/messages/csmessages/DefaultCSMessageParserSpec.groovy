...
import se.signatureservice.messages.dummy.DummyPayloadParser;
...
import se.signatureservice.messages.sysconfig.SysConfigPayloadParser;
...
public class DefaultCSMessageParserSpec extends Specification{
	...
	se.signatureservice.messages.sysconfig.jaxb.ObjectFactory sysConfigOf = new se.signatureservice.messages.sysconfig.jaxb.ObjectFactory()
	...
	def "Verify that generateGetApprovalRequest() generates a valid xml message  generateGetApprovalResponse() generates a valid CSMessageResponseData"(){
		...
		when:
		mp.parseMessage(TestMessages.getApprovalRequestWithInvalidRequestPayload)
	...
	def "Verify that getSigningCertificate parses signer certificate"(){
		expect:
		mp.requireSignature()
		mp.getSigningCertificate(TestMessages.simpleCSMessage) instanceof X509Certificate
		when:
		mp.requireSignature = false
		then:
		mp.getSigningCertificate(TestMessages.simpleCSMessage) == null

		when:
		mp.requireSignature = true
		mp.getSigningCertificate(TestMessages.simpleCSMessageWithBadCertificate)
		then:
		thrown MessageContentException
	...
	def "Verify validateCSMessage() method"(){
		...
		mp.validateCSMessage(mp.getVersionFromMessage(TestMessages.simpleCSMessage), mp.parseMessage(TestMessages.simpleCSMessage), getDoc(TestMessages.simpleCSMessage),true, true)
		then:
		true
		...
		mp.validateCSMessage(mp.getVersionFromMessage(TestMessages.cSMessageWithInvalidSignature), mp.parseMessage(TestMessages.cSMessageWithInvalidSignature), getDoc(TestMessages.cSMessageWithInvalidSignature), true, true)
		then:
		...
		mp.validateCSMessage(mp.getVersionFromMessage(TestMessages.simpleCSMessageWithInvalidPayload), mp.parseMessage(TestMessages.simpleCSMessageWithInvalidPayload), getDoc(TestMessages.simpleCSMessageWithInvalidPayload), true, true)
		then:
	...
	def "Verify that validateSignature correctly parses the ds:Signature object and verifies the signature"(){
		...
		when:
		mp.validateSignature(getDoc(TestMessages.simpleCSMessage), true, true)
		then:
		true
		when:
		mp.validateSignature(getDoc(TestMessages.cSMessageWithInvalidSignature), true, true)
		then:
		thrown MessageContentException
		
		when:
		mp.validateSignature(getDoc(TestMessages.simpleCSMessageWithoutSignature), true, true)
		then:
		thrown MessageContentException
	...
	def "Verify that getVersionFromMessage parses version and payload version from message"(){
		when:
		CSMessageVersion v = mp.getVersionFromMessage(TestMessages.simpleCSMessagePayloadVersion2_1)
		then:
	...
	def "Verify that getVersionFromMessage throws MessageContentException for invalid message data"(){
		...
		thrown MessageContentException
		where:
		message << [TestMessages.simpleCSMessageWithEmptyVersion, TestMessages.simpleCSMessageWithEmptyPayloadVersion, TestMessages.simpleCSMessageWithoutVersion, TestMessages.simpleCSMessageWithoutPayloadVersion, TestMessages.invalidXML]
	}
	...
	def "Verify that JAXB Related Data helper method works"(){
		...
		mp.jaxbData.jaxbClassPath =~ "se.signatureservice.messages.csmessages.jaxb"
		mp.jaxbData.jaxbClassPath =~ ":se.signatureservice.messages.sysconfig.jaxb"
		mp.jaxbData.jaxbContext != null
		...
		then:
		mp.jaxbData.jaxbClassPath =~ "se.signatureservice.messages.dummy.jaxb"
		mp.jaxbData.jaxbContext != null
		mp.jaxbData.jaxbIntrospector != null
